...
Sub auto_open()
    ...
    Set para_range = Workbooks(ThisWorkbook.name).Sheets("報表參數").Range("A1:E500")
        ...
        Next
        For i = 1 To 500
            If para_range.Cells(i, 5).value = "紀錄系統log" Then
...
Sub auto_close()
    ...
    If Not ActiveWorkbook.ReadOnly Then
        ...
        Set para_range = Workbooks(ThisWorkbook.name).Sheets("報表參數").Range("A1:E500")
            For j = 1 To 500
                If para_range.Cells(j, 5).value = "設定自動更新，開啟時會自動匯入最新資料" Then
                    If para_range.Cells(j, 4).value = "自動更新" Then
...
exit_sub:
                        '刪除所有message
                        For i = 1 To Workbooks(ThisWorkbook.name).Sheets.Count
                            If Workbooks(ThisWorkbook.name).Sheets(i).name Like "*參數*" Or Workbooks(ThisWorkbook.name).Sheets(i).name Like "*說明*" Then
...
Sub frm_rec_log_txt()
    ...
    CheckPath = Right(LocalPath, Len(LocalPath) - InStrRev(LocalPath, "\"))
    UpperPath = Left(LocalPath, InStrRev(LocalPath, "\") - 1) & "\"
    s_path = UpperPath & "廠務部報表_紀錄用\db\log_db.xlsb"
    t_path = "\\SolarDoc\Public\廠務部公告\log\WEA\log_db.xlsb"
    
    '開啟要寫入的DB，如果找不到檔案就複製一份
    TestStr = ""
    TestStr = Dir(t_path)
    If TestStr = "" Then
...
Sub import_data()
    ...
    '參數設定確認
    For k = 1 To 500
        If para_range.Cells(k, 5).value = "挑選資料模型後，再設定你需要的欄位與欄位順序" Then
    ...
    Next
    For k = 1 To 500
        If para_range.Cells(k, 5).value = "設定自動更新，開啟時會自動匯入最新資料" Then
            If para_range.Cells(k, 4).value = "" Then
                MsgBox "你沒有設定是否自動更新喔。"
                Exit Sub
            Else
    ...
    Else
        MsgBox "【" & spath_file(1) & "】 檔案不存在!"
    End If
    
    
    '轉貼資料模型
    Workbooks(ThisWorkbook.name).Activate
    Workbooks(ThisWorkbook.name).Sheets("刪除空白行列").Cells.ClearContents
    Windows(file_file(1)).Activate
    Sheets("db").UsedRange.Copy
    Workbooks(ThisWorkbook.name).Activate
    Workbooks(ThisWorkbook.name).Sheets("刪除空白行列").Select
    Range("A1").Select
    Selection.PasteSpecial Paste:=xlPasteValues, Operation:=xlNone, SkipBlanks:=False, Transpose:=False
    Range("A1").Copy
